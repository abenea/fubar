...
void myperror(std::string message) { qDebug() << message.c_str() << ": " << strerror(errno); }

DirectoryWatcher::DirectoryWatcher() : stop_(false) {
    fd_ = inotify_init();
    if (fd_ < 0)
...
void DirectoryWatcher::addWatch(QString path) {
    int wd = inotify_add_watch(fd_, path.toUtf8().constData(),
                               IN_CREATE | IN_DELETE | IN_DELETE_SELF | IN_MOVE /*| IN_IGNORED */ |
...
void DirectoryWatcher::handleEvent(inotify_event *event) {
    ...
    if (eventType != UNKNOWN && event->len != 0) {
        QString path =
            QFileInfo(QDir(it->second), QString::fromUtf8(event->name)).absoluteFilePath();
...
void DirectoryWatcher::dumpWatches() {
    for (WatchMap::right_const_iterator it = watches_.right.begin(); it != watches_.right.end();
         ++it)
