...
void PlaylistTab::addTracks(const QList<shared_ptr<Track>> &tracks) { model_->addTracks(tracks); }
...
void PlaylistTab::serialize(QByteArray &data) const { model_->playlist().serialize(data); }

void PlaylistTab::deserialize(const QByteArray &data) { model_->deserialize(data); }
