...
void AudioPlayer::setMainWindow(MainWindow *mainWindow) {
    ...
    QObject::connect(
        &config, SIGNAL(keySet(QString, QVariant)), this, SLOT(configChanged(QString, QVariant)));
}
...
void AudioPlayer::readSettings() {
    ...
    preamp_with_rg_ =
        settings.value("playback/replaygain.preamp_with_rg", preamp_with_rg_).toReal();
    playbackOrder_ =
        PlaybackOrder(settings.value("playback/order", static_cast<int>(playbackOrder_)).toInt());
...
void AudioPlayer::aboutToFinish() {
    ...
    if (playingModel_) {
        QModelIndex index =
            getNextModelIndex(playingModel_, playbackOrder_ == PlaybackOrder::RepeatTrack ? 0 : 1);
        if (!index.isValid())
            return;
...
void AudioPlayer::currentSourceChanged() {
    //     qDebug() << "currentSourceChanged() " << audioOutput_->currentTime() << " " <<
    //     audioOutput_->totalTime();
    if (queue_.peeked()) {
...
void AudioPlayer::slotTick(qint64 pos) {
    ...
    if (lengthHack_ && !playingTrack_->isCueTrack() &&
        playingTrack_->audioproperties.length + 1 < pos / 1000) {
        qDebug() << "Setting" << playingTrack_->path() << "length to " << pos / 1000 << "(was"
                 << playingTrack_->audioproperties.length << ")";
...
PTrack AudioPlayer::getCurrentTrack() { return playingTrack_; }

PModel AudioPlayer::getPlayingPlaylistModel() { return playingModel_; }

QModelIndex AudioPlayer::getPlayingIndex() { return playingIndex_; }
...
void AudioPlayer::prev() { playNext(-1); }
...
PModel AudioPlayer::createPlaylist(bool synced) {
    ...
    if (synced) {
        ...
        QObject::connect(library_,
                         SIGNAL(libraryChanged(LibraryEvent)),
                         model.get(),
                         SLOT(libraryChanged(LibraryEvent)));
        QObject::connect(library_,
                         SIGNAL(libraryChanged(QList<PTrack>)),
                         model.get(),
                         SLOT(libraryChanged(QList<PTrack>)));
    }
...
void AudioPlayer::play(PModel playlistModel, const QModelIndex &index) {
    ...
    if (currentlyPlayingTrack && audioOutput_->state() == AudioState::Playing &&
...
QModelIndex AudioPlayer::getNextModelIndex(PModel playlistModel, int offset) {
    ...
    if (playingModel_ == playlistModel && lastPlayedIndex_.isValid())
        return mainWindow_->getFilteredIndex(
            playingModel_, lastPlayedIndex_, offset == -1 ? 0 : offset);
...
void AudioPlayer::setVolume(qreal value) {
    ...
    if (replaygain_ != ReplayGainMode::None) {
        ...
        if (track) {
            // gain = 10 ^ ((rg + pream) / 20)
            auto id_tag = replaygain_ == ReplayGainMode::Album ? "REPLAYGAIN_ALBUM_GAIN"
            ...
            if (it != track->metadata.end()) {
                ...
                qDebug() << "Got" << replaygain_ << "replaygain " << track_rg
                         << "resulting in gain =" << rg;
            }
            modified_volume *= rg;
        }
    }
...
void AudioPlayer::slotFinished() {
    ...
    if (bufferingTrack_->isCueTrack() && bufferingTrack_->location == playingTrack_->location &&
        bufferingTrack_->metadata["track"].toInt() ==
            playingTrack_->metadata["track"].toInt() + 1) {
